...
sample1623CE_100<- sample1623CE_100 %>% mutate(average_yob=(judge_yob+judge_yob+judge_yob3)/3,
  ...
  ungroup() %>%
  pivot_longer(cols = c(judge1, judge2, judge3), names_to = "judge_role", values_to = "judge_id")
...
clustered_se_100 <- vcovCL(length_proceeding_100, cluster = ~ judge_id)
clustered_se_C_100 <- vcovCL(length_proceeding_C_100, cluster = ~ judge_id)
clustered_se_50 <- vcovCL(length_proceeding_50, cluster = ~ judge_id)
clustered_se_C_50 <- vcovCL(length_proceeding_C_50, cluster = ~ judge_id)
clustered_se_20 <- vcovCL(length_proceeding_20, cluster = ~ judge_id)
clustered_se_C_20 <- vcovCL(length_proceeding_C_20, cluster = ~ judge_id)

# Repeat for the other models
clustered_se_outcome_100 <- vcovCL(outcome_100, cluster = ~ judge_id)
clustered_se_outcome_C_100 <- vcovCL(outcome_C_100, cluster = ~ judge_id)
clustered_se_outcome_50 <- vcovCL(outcome_50, cluster = ~ judge_id)
clustered_se_outcome_C_50 <- vcovCL(outcome_C_50, cluster = ~ judge_id)
clustered_se_outcome_20 <- vcovCL(outcome_20, cluster = ~ judge_id)
clustered_se_outcome_C_20 <- vcovCL(outcome_C_20, cluster = ~ judge_id)

clustered_se_cpy_100 <- vcovCL(cited_per_year_100, cluster = ~ judge_id)
clustered_se_cpy_C_100 <- vcovCL(cited_per_year_C_100, cluster = ~ judge_id)
clustered_se_cpy_50 <- vcovCL(cited_per_year_50, cluster = ~ judge_id)
clustered_se_cpy_C_50 <- vcovCL(cited_per_year_C_50, cluster = ~ judge_id)
clustered_se_cpy_20 <- vcovCL(cited_per_year_20, cluster = ~ judge_id)
clustered_se_cpy_C_20 <- vcovCL(cited_per_year_C_20, cluster = ~ judge_id)
