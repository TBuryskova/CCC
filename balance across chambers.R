...
p_n_applicants <- get_p_value(aov(n_applicants ~ chamber_id_alt, data_clean))

ggplot(balance_applicants, aes(x = chamber_id_alt, y = mean_n)) +
...
p_n_acts <- get_p_value(aov(n_concerned_act ~ chamber_id_alt, data_clean))

ggplot(balance_acts, aes(x = chamber_id_alt, y = mean_n)) +
  geom_bar(fill = "skyblue", stat = 'identity') +
  geom_errorbar(aes(ymin = mean_n - se_n, ymax = mean_n + se_n), width = 0.2) +
...
p_controversial <- get_p_value(aov(controversial ~ chamber_id_alt, data_clean))

ggplot(balance_controversial, aes(x = chamber_id_alt, y = mean_n)) +
  geom_bar(fill = "skyblue", stat = 'identity') +
  geom_errorbar(aes(ymin = mean_n - se_n, ymax = mean_n + se_n), width = 0.2) +
...
p_dow <- get_p_value(aov(dow ~ chamber_id_alt, dow))

ggplot(dow, aes(x = factor(dow), fill = chamber_id_alt)) +
...
ggplot(balance_cases, aes(chamber_id_alt, n)) +
...
p_monthly_cases <- get_p_value(aov(n_cases ~ chamber_id_alt, data = monthly_cases))

# Plot
ggplot(balance_monthly, aes(x = chamber_id_alt, y = mean_monthly_cases)) +
